...
public class Missile
{
	...
	//必杀技清掉子弹
	public boolean clearMissile(List<EmpMissile> emslist)
	{
		for (int i = 0; i < emslist.size(); i++) 
		{
			if (this.b_live && emslist.get(i).em_live && this.ms2Reg().intersects(emslist.get(i).emsReg()))
			{
				emslist.get(i).em_live = false;
	...
	//必杀技清掉飞机
	public boolean clearPlane(List<NewPlane> newlist)
	{
		for (int i = 0; i < newlist.size(); i++)
		{
			if (this.b_live && newlist.get(i).e_live && this.ms2Reg().intersects(newlist.get(i).empReg()))
			{
				newlist.get(i).e_live = false;

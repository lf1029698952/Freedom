...
public class GameStart extends Frame
{
	...
	Image img_1 = tk.getImage(GameStart.class.getResource("/image/gamestart.jpg"));
	Image img_2 = tk.getImage(GameStart.class.getResource("/image/gameover.jpg"));
	...
	public void paint(Graphics g)
	{
		...
		case 1:
			...
			g.drawString("杀敌数：" + num, 30, 90);
			//g.drawString("敌军集合数目：" + emplist.size(), 30, 120);
			myPlane.drawMe(g);
			bldMe.drawMe(g);
			...
			for (int i = 0; i < mslist1.size(); i++)
			{
				...
				{
					num++;
					ex = new Explode(mslist1.get(i).m1_x, mslist1.get(i).m1_y, true, this);
					ex.draw(g);
				}
				if (ms.hitBoss(boss))
				{
					num++;
					ex = new Explode(mslist1.get(i).m1_x+40, mslist1.get(i).m1_y, true, this);
					ex.draw(g);
				}
			}
			for (int i = 0; i < mslist2.size(); i++)
	...
	public void getEmp()
	{
		...
		{
			EmpPlane emp = new EmpPlane(e_x, random.nextInt(570)+20, true, Direction.L, 300, this);
			emplist.add(emp);
		}
	}

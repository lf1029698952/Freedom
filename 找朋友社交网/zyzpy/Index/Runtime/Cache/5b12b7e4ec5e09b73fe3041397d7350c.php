	...
	<div class='row'>
		...
		<!--中间开始-->
			<div class="col-md-7 white">
				<h3 class="text-muted text-center"><span class="glyphicon glyphicon-envelope"></span> 我的私信（共<?php echo ($count); ?>条）<button class='btn btn-warning btn-sm' data-toggle="modal" data-target="#letter" >发送私信</button></h3>
				<hr/>
					<!--私信开始-->
					<?php if($letter): if(is_array($letter)): foreach($letter as $key=>$v): ?><div class="row letter">
							<div class="col-md-2">
								<a href="<?php echo U('/'.$v['uid']);?>" target="_blank">
								<img src="
								<?php if($v["face"]): ?>__ROOT__/Uploads/Face/<?php echo ($v["face"]); ?>
								<?php else: ?>
									__PUBLIC__/Them/<?php echo ($style); ?>/images/1.jpg<?php endif; ?>" width="50px" height="50px"/></a>
							</div>
							<div class="col-md-7">
								<p class="username"><a href="<?php echo U('/'.$v['uid']);?>" target="_blank"><?php echo ($v["username"]); ?></a>：<?php echo (replace_weibo($v["content"])); ?></p>
								<p class="text-muted"><?php echo (time_format($v["time"])); ?> </p>
							</div>
							<div class="col-md-3">
								<span class='del-letter text-primary pointer' id='<?php echo ($v["id"]); ?>'>删除</span>
								| 
								<span class='reply-letter text-primary pointer' username='<?php echo ($v["username"]); ?>' data-toggle="modal" data-target="#letter">回复</span>
							</div>
						</div>
						<br/><?php endforeach; endif; ?>
					<p class="text-center"><?php echo ($page); ?></p>
					<?php else: ?>
						<h2 class="text-center text-info">木有小伙伴给我发私信<img src="__PUBLIC__/biaoqing/xiaoku.gif" alt="小哭" /></h2><?php endif; ?>
					<!--私信结束-->
					<hr/>
			</div>
		<!--中间结束-->
